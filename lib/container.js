...
proto.mount = function (prefix, app) {
  ...
  this._mount(prefix, app);
  // this.subapps.push([prefix, app]);
};

proto._mount = function (prefix, app) {
  // var _mw = _.slice(app.middleware, 0);
  var _mw = app.middleware;
  //namespace support
  _mw.unshift(function *(next) {
    var redirect = this.redirect;
    this.redirect = function (url, alt) {
      if(url.indexOf('http') !== 0 && url.indexOf(prefix) !== 0) {
        url = prefix + url;
  ...
  this.use(function *(next) {
    var m = mount(prefix, compose(_mw));
